...
(defmacro def-hooked-macro (name args &body body)
   (multiple-value-bind (hookname args body)
      ...
      (multiple-value-bind (env-var d-args)
                           (let ((evtl (member '&env macro-args)))
                              (cond (evtl
                                     (values (cadr evtl)
                                    ...
                                    (t
                                     (values (gen-var 'env)
                                             args))))
         (multiple-value-bind (whole-var macro-args)
                              (cond ((car-eq d-args '&whole)
                                     (values (cadr d-args) (cddr d-args)))
                                    (t
                                     (values (gen-var 'whole) d-args)))
            `(defmacro ,name (&whole ,whole-var ,@macro-args
                              &environment ,env-var)
                (check-for-macro-hook
                   ',hook-name ,whole-var ,env-var
                   (\\ (,whole-var ,env-var)
                      (declare (ignorable ,env-var))
                      (destructuring-bind ,d-args ,whole-var
                         ,@body))))))))
